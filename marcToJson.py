...
def marc_json_to_dict(json):
    ...
    for marcEntry in json:
        ...
        for obj in titleSubfields:
            try:
                title = obj['a'][:-2] # every title had a space and / after it
            except:
                pass
        ...
        for obj in authorSubfields:
            try:
                author = obj['a'][:-1] # weird trailing comma issue
            except:
                pass
        ...
        bookEntry['genre'] = genre[:-1]

    
        imageSubfields = safeGetField(fieldsObj, ['856', 'subfields'])
        for obj in imageSubfields:
            try:
                url = obj['u']
            except:
                pass
        bookEntry['coverImageUrl'] = url
...
def main():
    ...
    # print jsonObj
    result = marc_json_to_dict(jsonObj)

    saveFile = open(sys.argv[2], 'wb')
    json.dump(result, saveFile, sort_keys=True, indent=4)
    saveFile.close()
    f.close()
